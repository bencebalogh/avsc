...
Parser.prototype._readProtocol = function () {
  ...
  this._readDecorations(attrs);
  tk.next({val: 'protocol'});
  if (tk.next().val !== '{') {
    attrs.protocol = tk.prev().next({id: 'name'}).val;
    tk.next({val: '{'});
  }
  var annotations;
  while (tk.next().val !== '}') {
    if (tk.prev().next().val === 'import') {
      this._imports.push(this._readImport());
    } else {
      tk.prev();
      var typeAttrs = this._readType(annotations);
      annotations = {};
      var numImports = this._readDecorations(annotations, true);
      var name = tk.next().val; // Potential message name.
      if (tk.next().val === '(') {
        // We are reading a message.
        if (numImports) {
          throw new Error(f('unexpected import in message %s', name));
        }
        hasMessage = true;
        var oneWay = false;
        ...
        annotations.response = typeAttrs;
        this._readMessage(annotations);
        if (oneWay) {
          annotations['one-way'] = true;
        }
        if (messages[name]) {
        ...
        messages[name] = annotations;
        annotations = undefined;
      } else {
        // This was a standalone type definition. Note that we had also already
        // started reading the next type, so keep the annotations to grant them
        // to the next type.
...
Parser.prototype._readImport = function () {
  ...
  var importAttrs = {kind: kind, name: fname};
  tk.next({val: ';'});
  return importAttrs;
};
...
Parser.prototype._readMessage = function (attrs) {
  // Tokenizer should be just after the open parens.
  var tk = this._tk;
  if (this._reassignJavadoc) {
    attrs.response = reassignJavadoc(attrs.response, attrs);
  }
  attrs.request = [];
  if (tk.next().val !== ')') {
    tk.prev();
...
Parser.prototype._readDecorations = function (attrs, maybeMessage) {
  var tk = this._tk;
  var numImports = 0;
